...
async function main() {
  ...
  function checkGestureCombination(closeHand, poseData) {
    const addToPairIfCorrect = (closeHand) => {
      const containsHand = poseData.some(finger => dont[closeHand].includes(finger[2]))
      if(!containsHand) return;
      pair.add(closeHand)
    }

    addToPairIfCorrect(closeHand)
    console.log(pair);
  ...
  const estimateHands = async () => {
    ...
    for (const hand of hands) {
      ...
      if(!predictions.gestures.lenght) {
